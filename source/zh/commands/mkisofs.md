...
创建ISO 9660/HFS混合CD。此选项应与-map、-magic或下列各种双破折号选项结合使用。

-apple

创建带有Apple扩展的ISO 9660 CD。类似于-hfs，只是添加了对ISO 9660的苹果扩展，而不是创建一个HFS混合卷。以前的genisoImage版本在默认情况下包括Rock Ridge属性，如果“-apple”是指定的。这个版本的genisoImage不再这样做了。如果您希望拥有RockRidge属性，则需要分别指定此属性。
